...
import { languageCode, translateMessage } from './i18n'
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background px-6 text-foreground">
      <section className="flex w-full max-w-md flex-col items-center rounded-2xl border border-border bg-surface p-10 text-center shadow-sm">
        <img alt={t('Rato, household finance')} className="mb-5 h-12 w-auto" height="56" src={`${import.meta.env.BASE_URL}logo.svg`} width="204" />
        <h1 className="mt-3 text-2xl font-semibold tracking-tight">{t('Your finances, locally')}</h1>
        <p className="mt-3 text-sm text-muted" role="status" aria-live="polite">
          {t('Loading your locally stored data…')}
        </p>
...
function RecoveryScreen() {
  ...
  const reset = async () => {
    const confirmed = window.confirm(t('Reset Rato on this device? This removes the saved data from this browser and creates a new empty baseline.'))
    if (!confirmed) return
    setResetting(true)
    setResetError(null)
    try {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background px-6 text-foreground">
      <section className="w-full max-w-lg rounded-2xl border border-border bg-surface p-8 shadow-sm sm:p-10">
        ...
        <img alt={t('Rato, household finance')} className="mt-6 h-10 w-auto" height="56" src={`${import.meta.env.BASE_URL}logo.svg`} width="204" />
        <h1 className="mt-2 text-2xl font-semibold tracking-tight">{t('Your saved data needs attention')}</h1>
        <p className="mt-3 text-sm leading-6 text-muted" role="alert">
          {t('Rato could not read the saved data in this browser. It has been left untouched. You can retry loading it or reset this device to a new empty baseline.')}
        </p>
        {resetError && <p className="mt-3 text-sm text-danger" role="alert">{translateMessage(resetError, t)}</p>}
        <div className="mt-7 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            ...
            disabled={resetting}
          >
            {resetting ? t('Resetting…') : t('Reset local data')}
          </button>
        </div>
        <p className="mt-5 text-xs text-muted">
          {t('Reset removes this app’s saved data from the current browser only.')}
        </p>
      </section>
    </main>
  )
...
export default function App() {
  ...
  useEffect(() => {
    const language = languageCode(activeI18n.resolvedLanguage ?? activeI18n.language)
    document.documentElement.lang = language
    document.querySelector('meta[name="description"]')?.setAttribute('content', t('A local-first household finance dashboard.'))
